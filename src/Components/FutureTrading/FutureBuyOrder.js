...
const FutureBuyOrder = () => {
  ...
  const [futureBuyOrders, setFutureBuyOrders] = useState({});
  ...
  const getFutureBuyOrders = async () => {
    try {
      ...
      if (data?.success == true) {
        setFutureBuyOrders(data?.result)
      } else {
        setFutureBuyOrders({});
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    setCollections({
      ...
      row: formattedRow,
      pagecount: futureBuyOrders?.totalPages
    });
  }, [futureBuyOrders]);
          ...
          <Grid2 container spacing={2}>
            <Grid2 item size={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }}>
              <div className='display-2 mb-10'>
                <h2 style={{ margin: 0 }}>Futures Buy Order Lists</h2>
              </div>
