...
const SpotSellOrder = () => {
  ...
  const [spotSellOrders, setSpotSellOrders] = useState({});
  ...
  const handleSearch = () => {
    setPage(1);
    setCurrencyPairs(searchRef.current?.value)
  };
  ...
  //       setCurrencyPairs(data?.result);
  //       console.log(data)
  //     } else {
  //       setCurrencyPairs(["All"]);
  //     }
  //   } catch (error) {
  //     setCurrencyPairs(["All"]);
  //     console.log(error);
  ...
  const getSpotSellOrders = async () => {
    try {
      const { data } = await Axios.post(
        `/admin/getspotorderslist?limit=10&page=${page}`,
        { type: 'spot', pair: currencyPairs, order_type: orderType, side: 'sell' },
        {
          headers: {
            Authorization: localStorage.getItem("nzanzi"),
          },
  ...
  useEffect(() => {
    setCollections({
      ...
      row: spotSellOrders?.docs,
      pagecount: spotSellOrders?.totalPages
    });
  }, [spotSellOrders]);

  const orderTypesArray = ["All", "Limit", "Market", "Stop-Limit"];
          ...
          <Grid2 container spacing={2}>
            <Grid2 item size={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }}>
              <div className='display-2 mb-10'>
                <h2 style={{ margin: 0 }}>Spot Sell Order Lists</h2>
              </div>
              ...
              {/* <Box sx={fieldStyle}>
                    <FormControl fullWidth>
                      ...
                      >
                        {orderTypesArray.map((pair,pairIndex)=><MenuItem key={pairIndex} value={pair}>{pair}</MenuItem>)}
                      </Select>
                    </FormControl>
                  </Box>
                  <Box sx={fieldStyle}>
                    <div class='group'>
                      ...
                      <Button variant="contained" 
                        className="action-btn" onClick={handleSearch}>
                        Search</Button>
                    </div>
                  </Box> */}
              {/* <Box sx={fieldStyle}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        value={currencyPairs}
                      >
                        {currencyPairs.map((pair,pairIndex)=><MenuItem key={pairIndex} value={pair}>{pair}</MenuItem>)}
                      </Select>
                    </FormControl>
                  </Box> */}
              {/* <Box sx={fieldStyle}>
